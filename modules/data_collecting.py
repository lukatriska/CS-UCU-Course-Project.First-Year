    ...
    for month in months:
        for hour in hours:
            ...
            for i in range(len(coors_list)):
                ...
                with open('{}_{}_data{}.txt'.format(month, hour, i+1), 'w') as outfile:
                    ...
                    # outfile.write(str(coors_to_address(coors_list[i][0]))+'\n)
                    # outfile.write(str(coors_to_address(coors_list[i][1]))+'\n)
                    # outfile.write(str(route_data['routes'][0]['legs'][0]['steps'][i]['distance']['text'])+'\n')
                    # outfile.write(str(route_data['routes'][0]['legs'][0]['steps'][i]['duration']['text'])+'\n')
                    outfile.write(str(weather_data['currently']['summary'])+'\n')

            # filenames.append('data{}.txt'.format(i+1))
            # with open('data{}.txt'.format(i+1), 'w') as outfile:
...
# with open('from_{}_to_{}.txt'.format(point_A, point_B), 'w') as outfile:
